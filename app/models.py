...
class User(db.Model):
    ...
    subscribers = db.relationship("Subscribe", back_populates='subscribers', cascade="all, delete-orphan",
                                foreign_keys="Subscribe.subscriber_id")
    targets = db.relationship("Subscribe", back_populates='targets', cascade="all, delete-orphan",
...
#
#     id = db.Column(db.Integer, primary_key=True, autoincrement=True)
#     tweet_id = db.Column(db.Integer, db.ForeignKey("tweet.id", ondelete='CASCADE'))
...
class Tweet(db.Model):
    ...
    medias = relationship('Media', secondary=tweet_media, back_populates='tweets')
...
class Subscribe(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    subscriber_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='CASCADE'))
    target_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='CASCADE'))

    __table_args__ = (
        db.UniqueConstraint('subscriber_id', 'target_id', name='uq_subscriber_target'),
        db.Index('idx_subscriber', 'subscriber_id'),
        db.Index('idx_target', 'target_id'),
    )

    subscribers = db.relationship("User", foreign_keys=[subscriber_id], back_populates='subscribers')
    targets = db.relationship("User", foreign_keys=[target_id], back_populates='targets')

    def __repr__(self):
        return f"Follow(subscriber_id={self.subscriber_id}, target_id={self.target_id})"
...
class Media(db.Model):
    ...
    tweets = relationship('Tweet', secondary=tweet_media, back_populates='medias')
